...
if (isset($_POST["username"]) && isset($_POST["password"]) && isset($_POST["password_2"]) && isset($_POST["email"])) {
    ...
    $res_name = mysql_query("SELECT `id` FROM `user` WHERE `username` = '" . strtolower($username) . "'");
    $res_mail = mysql_query("SELECT `id` FROM `user` WHERE `email` = '$email'");
    $rows_name = mysql_num_rows($res_name);
    $rows_mail = mysql_num_rows($res_mail);
