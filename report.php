...
function getNumberOfCrashes() {
	...
	$res_overall_crashnr = mysql_query($sql_overall_crashnr);
	return mysql_num_rows($res_overall_crashnr);
}

function showNumberOfCrashes() {
	$sql_overall_crashnr = create_mysql_select(null, "issue_id = '?'", array($_GET[issue_id]));
	$res_overall_crashnr = mysql_query($sql_overall_crashnr);
	$rows = mysql_num_rows($res_overall_crashnr);
	if ($rows == 0) {
		header("location: index.php");
		exit;
	}
...
<div class="rightreportcolumn">
	<div id="tabs">
	    ...
	    <div id="tabs-1">
		  	<p><?php
		  			if ($rows == 1) {
		  				$tab2 = mysql_result($res, 0, 'stack_trace');
		  				echo "<pre>$tab2</pre>";

		  			} else {
		  				$tab2 = mysql_fetch_assoc($res);
		  				showStacktrace($tab2);
		  			}
		  		?>
		  	</p>
	    ...
	    <div id="tabs-2">
			...
			<?php
				...
				$res_versions = mysql_query($sql_versions);

			  	while ($tab3 = mysql_fetch_assoc($res_versions)) {
			  		showVersions($tab3);
			  		$sql_crashes = "SELECT * FROM `crashes` WHERE `issue_id` = '".$_GET[issue_id]."' AND `app_version_code` = '".$tab3['app_version_code']."'";
			  		$res_crashes = mysql_query($sql_crashes);
			  		$crashes = mysql_num_rows($res_crashes);
			  		echo "<td>$crashes</td>
					</tr>";
			  	}
		  	?>
